...
export function render({ app, dbService, escapeHtmlText, formatDisplayTimestamp, params } = {}) {
    ...
    const rows = items.map(c => {
        ...
        const confirmLabel = isConfirmedStatus(c.status) ? '履歴を見る' : '確認する';
        const confirmButton = `
                <button onclick='event.stopPropagation(); window.app.navigate("diff", ${JSON.stringify({ id: c.id, source: 'contracts' })})'
                        style="background:#fff; color:#0b2d62; border:1px solid #cbd5e1; border-radius:6px; padding:6px 12px; font-size:11px; font-weight:700; cursor:pointer; white-space:nowrap; display:inline-flex; align-items:center; justify-content:center; gap:6px; width:100%; transition:all 0.2s;" onmouseover="this.style.background='#f8fafc'; this.style.borderColor='#0b2d62';" onmouseout="this.style.background='#fff'; this.style.borderColor='#cbd5e1';">
                    <i class="fa-solid fa-eye"></i> ${confirmLabel}
                </button>
            `;

        const actionButton = `
                <button onclick='window.app.startDiffCheckFromContract(${JSON.stringify(c.id)})'
                    ...
                    <i class="fa-solid fa-code-compare"></i> 差分チェックする
                </button>
            `;
        ...
        return `
                <tr onclick='window.app.navigate("diff", ${JSON.stringify({ id: c.id, source: 'contracts' })})' style="cursor:pointer;">
                    ...
                    <td>${riskBadge}</td>
                    <td>${statusBadge}</td>
                    <td onclick="event.stopPropagation()" style="width:140px; text-align:center; vertical-align:middle;">
