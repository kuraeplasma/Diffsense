...
4. 契約改善提案(changes): 取り込んだ契約書を「リスク削減」+「不足条項の補完」+「不適切表現の修正」の3観点から総合的に分析し、 契約書としての完成度を高める提案を最大5件 出力してください。
   ...
   【提案の精度要件】
   - 契約書全体を熟読し、 各条項の文脈・前後関係・相互参照を理解した上で提案すること
   - 業界慣行および関連法令 (民法、 特定商取引法、 個人情報保護法、 下請法、 独占禁止法、 労働法 など適用領域に応じた法律) を踏まえること
   - 抽象的・一般論的な助言ではなく、 具体的に書き換え可能な条文テキストを new フィールドに記載すること
   - 提案根拠 (concern) は法的観点で明確に示し、 「なぜこの修正が必要か」 を商用法務担当者にも納得できる粒度で記述すること
   - 既に十分な条項が存在する場合は無理に提案を増やさず、 件数より質を優先すること (最大5件、 質を担保できなければ少なくて可)
