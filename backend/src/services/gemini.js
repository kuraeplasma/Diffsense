...
4. 変更点(changes): 契約書を解析し、リスク削減・不足条項補完・表現修正・明確化の観点で最大5件提案してください。 同じタイプばかりに偏らず、契約の実態に応じて適切なタイプを選択してください。
  ...
  "changes": [
    {
      ...
      "old": "書類内の該当箇所の文章をそのまま一字一句引用（要約・言い換え禁止）。 addition (条項追加) の場合は空文字",
      "new": "修正後の契約条項文言（解説や要約、箇条書き、冒頭の見出しなどは一切含めず、契約書としてそのまま利用可能な正式な条文テキストのみを出力してください）。 risk で削除推奨時のみ空文字可",
      "impact": "法的影響",
      "concern": "注意点"
    }
  ],
