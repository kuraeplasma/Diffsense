...
4. 契約改善提案(changes): 取り込んだ契約書を「リスク削減」+「不足条項の補完」+「不適切表現の修正」の3観点から総合的に分析し、 契約書としての完成度を高める提案を最大5件 出力してください。
   - リスク削減(risk): 既存条項に法的/業務的リスクがあり、 削除または書き換えが必要なもの → type: "risk" (oldに削除/修正対象を、 newに修正案、 削除推奨時はnewを空に)
   - 不足条項補完(addition): 契約書に欠落している重要条項 (秘密保持、 解除事由明確化、 損害賠償上限など) を新規追加 → type: "addition" (oldは空、 newに追加条項全文)
   - 表現修正(modification): 既存条項の表現があいまい・誤解を招く場合、 より適切な文言に置換 → type: "modification" (oldに該当原文、 newに修正後文言)
   - 明確化(clarification): 解釈の余地がある条項を明確化する補足を追加 → type: "clarification" (oldは原文、 newに補足追加版)
   特定の type に偏らず、 契約書の実態に応じて適切な提案バランスで出力してください。
   【提案の精度要件 — 必ず守ること】
   ★ 解析プロセス: まず契約書全文を最後まで通読 → 契約類型(業務委託/秘密保持/雇用/売買/賃貸借/ライセンス/個人情報共同利用 等)を判定 → 適用される法令と業界慣行を確定 → そのうえで具体的な不備・リスク・改善点を洗い出す という手順を内部で実行してから出力すること。
   ★ 契約書を読まずに一般論で答えるのは厳禁。 一般的なテンプレ提案や定型句の羅列は絶対に避けること。
   ★ 各提案は契約書の実際の条文・文脈に即した固有のものとすること。 「秘密保持条項を追加」 のような汎用例ではなく、 「本契約第3条で受託者が知り得る個人情報の取扱いについて、 漏洩時の通知義務・損害賠償上限・委託先監督責任を以下のとおり明記」 のように 契約固有の文脈を踏まえた具体提案を出すこと。
   - 業界慣行および関連法令 (民法、 特定商取引法、 個人情報保護法、 下請法、 独占禁止法、 労働法 など適用領域に応じた法律) を踏まえること
   - 抽象的・一般論的な助言ではなく、 具体的に書き換え可能な条文テキストを new フィールドに記載すること (実際にコピー&ペーストで契約書に使える完成形)
   - 提案根拠 (concern) は法的観点で明確に示し、 「なぜこの修正が必要か」 を商用法務担当者にも納得できる粒度で記述すること (根拠条文・想定リスクシナリオ・実害例を含む)
   - ハルシネーション禁止: 契約書に存在しない条項を「修正対象」 として捏造しないこと。 oldフィールドは必ず契約書から一字一句正確に引用すること
   - 件数より質を優先すること (最大5件、 質を担保できなければ少なくて可。 1件しかない場合でも質の高い1件で可)
...
出力形式(JSONのみ):
{
  ...
  "changes": [
    {
      "section": "条項名",
      "type": "modification | risk | addition | clarification",
      "old": "書類内の該当箇所の文章をそのまま一字一句引用（要約・言い換え禁止）。 type:addition の場合は空文字。",
      "new": "修正後の契約条項文言（解説や要約、箇条書き、冒頭の見出しなどは一切含めず、契約書としてそのまま利用可能な正式な条文テキストのみを出力してください）。 type:risk で削除推奨時のみ空文字可。",
      "impact": "法的影響",
      "concern": "注意点"
    }
  ],
...
修正不要の場合:
{
  ...
  "summary": "主要条項に大きな問題はなく修正不要と判断（例：「損害賠償・秘密保持・解除条件が標準的な内容のため修正不要」のように修正不要の根拠を60文字以内で端的に）",
  "riskLevel": 1,
...
{
  "needs_revision": true,
  "summary": "修正が必要な理由を端的に（例：「損害賠償の上限未設定・秘密保持の範囲が曖昧なため修正を推奨」のように契約の説明ではなく修正理由を60文字以内で記載）",
  "riskLevel": 2,
  "riskReason": "リスク判定の根拠（100文字以内）",
